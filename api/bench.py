...
questions = [
    ...
    "Why do we have a spinal cord?", "Why do we have nerves?", "Why do we have a circulatory system?",
...
def calc_performance(max_concurrent_requests, file_name):
    C = max_concurrent_requests
    N = max_concurrent_requests * 4
    start_time = time.time()
    ...
    with open(file_name, 'a') as f:
        f.write(f'{C},{N},{total_time:.2f},{avg_time_per_request:.2f},{tokens_per_second:.2f}\n')


if __name__ == '__main__':
    warm_up()
    for i in range(loop):
        print(f'Starting test loop {i + 1}...')
        file_name = f'{model_name}_{i + 1}.csv'
        with open(file_name, 'w') as f:
            f.write('Max Concurrent Requests,Total Requests,Total Time,Average Time (s),Tokens Per Second\n')

        for max_concurrent_requests in max_concurrent_requests_list:
            print(f'Running performance test with {max_concurrent_requests} concurrent requests...')
            calc_performance(max_concurrent_requests, file_name)
