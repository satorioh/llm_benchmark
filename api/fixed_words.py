...
# number_of_requests_list = [1, 2, 4, 8, 10, 14, 16, 18, 20]
number_of_requests_list = [1, 2, 4, 8, 10, 14, 16, 18, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60]
...
if __name__ == "__main__":
    ...
    print(tabulate(results, headers=["Concurrent Request Number", "World Count", "Average Tokens", "Average Time (s)",
                                     "Average Speed (tokens/s)"]))

    # 将results中数据保存到result.csv中
    with open('result.csv', 'w') as f:
        f.write(tabulate(results, headers=["Concurrent Request Number", "World Count", "Average Tokens", "Average Time (s)",
                                           "Average Speed (tokens/s)"]))
        f.close()
